...
class AkkaHttpBackendL(
    delegate: SttpBackend[Future, Source[ByteString, Any], NothingT]
  ) extends SttpBackend[Task, Source[ByteString, Any], NothingT] {
  override def send[T](
  ...
  override def responseMonad: MonadError[Task] = new MonadError[Task] {
    ...
    override protected def handleWrappedError[T](
        ...
        h: PartialFunction[Throwable, Task[T]]
      ): Task[T] =
      rt.recoverWith(h)
  }

  override def openWebsocket[T, WS](
      request: Request[T, Source[ByteString, Any]],
      handler: NothingT[WS]
    ): Task[WebSocketResponse[WS]] = handler
}
